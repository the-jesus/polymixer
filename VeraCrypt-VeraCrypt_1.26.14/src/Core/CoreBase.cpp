...
namespace VeraCrypt
	...
	void CoreBase::ChangePassword (shared_ptr <Volume> openVolume, shared_ptr <VolumePassword> newPassword, int newPim, shared_ptr <KeyfileList> newKeyfiles, bool emvSupportEnabled, FilesystemPath Saltfile , shared_ptr <Pkcs5Kdf> newPkcs5Kdf, int wipeCount) const
	{
		...
		char buffer[bufferLen];		
	        if (! string(FilePath (Saltfile).ToBaseName()).empty()) {
			 //printf("Loading Salt:");
                         wipeCount=1;
	        	 //std::cout << string(FilesystemPath (Saltfile)); 
	        	 std::string target=string(FilesystemPath (Saltfile));
	        	 char* chr = strdup(target.c_str());
	 	      	 // char *cstr = new char[string(FilesystemPath (Saltfile)).size() + 1];
	 	      	 // const char *fname = string(FilesystemPath (Saltfile)).data();
	 	      	 // std::cout << chr; 
	 	      	 FILE * filp = fopen(chr , "rb"); 
	 	      	 bytes_read = fread(buffer, sizeof(char), 64, filp);
/*
			// Open the file 
				std::ifstream file(Saltfile);      
			// Check if the file was opened successfully 
			if (!file) { 
		        	throw std::runtime_error("Could not open the file: " + string(FilePath (Saltfile).ToBaseName())); 
			} 
			Use a stringstream to read the file contents 
			std::stringstream saltstring; 
			saltstring << file.rdbuf(); // Read the ent
*/
		}
		...
		while (true)
		{
			for (int i = 1; i <= wipeCount; i++)
			{
				if (bytes_read > 0 )
				{
					size_t bufferLen = newSalt.Size();
				        uint8* pbBuffer = new uint8[bufferLen];
				        //int ReadOffset;
					//bufferLen -= 64;
/*
				        // Initialize zero
...
//			                        ReadOffset = 0;
				...
				else
				{
					if (i == wipeCount)
					{
						...
						// std::cout << "The number is " << wipeCount << " and the string is " << newSalt[0] << std::endl; 
					}
					else
					{
